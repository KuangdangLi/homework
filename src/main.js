function isNumeric(value) {
    return typeof value === 'number' && !isNaN(value);
        ...
        let netMarketingOri = window.prompt("请输入你的网络营销考试分数");
        let onlineShopOri = window.prompt("请输入你的网店运营考试分数");
        isSpace(netMarketingOri)
        isSpace(onlineShopOri)
        const netMarketing = Number(netMarketingOri)
        const onlineShop = Number(onlineShopOri)
        let a = 15
        let b = 5
        if (isNumeric(netMarketing) && isNumeric(onlineShop) && netMarketing >= 0 && onlineShop >= 0 && netMarketing <=100 && onlineShop <=100){
            ...
            window.alert(`网络营销抄${a}遍,网店运营抄${b}遍`)
}else{
